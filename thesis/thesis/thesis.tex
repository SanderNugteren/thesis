\documentclass{article}
\usepackage{url}
\usepackage{graphicx, color}
\usepackage[a4paper,margin=2cm]{geometry}
\usepackage{tikz}
\usetikzlibrary{arrows.meta,positioning,calc}
\usepackage{qtree}
\usepackage[title]{appendix}

\newcommand{\red}[1]{{\color{red}{#1}}}
\let\oldsection\section
\renewcommand\section{\clearpage\oldsection}

\begin{document}


\begin{titlepage}

\newcommand{\HRule}{\rule{\linewidth}{0.5mm}} % Defines a new command for the horizontal lines, change thickness here
\center % Center everything on the page
 
%----------------------------------------------------------------------------------------
%	HEADING SECTIONS
%----------------------------------------------------------------------------------------

\includegraphics[width=\linewidth]{../images/uvaENG}\\[2.5cm]
\textsc{\Large MSc Artificial Intelligence}\\[0.2cm]
\textsc{\normalsize Track: Learning Systems}\\[1.0cm] % track
\textsc{\Large Master Thesis}\\[0.5cm] 

%----------------------------------------------------------------------------------------
%	TITLE SECTION
%----------------------------------------------------------------------------------------

\HRule \\[0.4cm]
{ \huge \bfseries Extracting rules from story synopses}\\[0.4cm] % Title of your document
\HRule \\[0.5cm]
 
%----------------------------------------------------------------------------------------
%	AUTHOR SECTION
%----------------------------------------------------------------------------------------

by\\[0.2cm]
\textsc{\Large Sander Nugteren}\\[0.2cm] %you name
6042023\\[1cm]


%----------------------------------------------------------------------------------------
%	DATE SECTION
%----------------------------------------------------------------------------------------

{\Large \today}\\[1cm] % Date, change the \today to a set date if you want to be precise

42 EC\\ %
June 2016\\[1cm]%

%----------------------------------------------------------------------------------------
%	COMMITTEE SECTION
%----------------------------------------------------------------------------------------
\begin{minipage}[t]{0.4\textwidth}
\begin{flushleft} \large
\emph{Supervisor:} \\
Dr. F.M. \textsc{Nack}% Supervisor's Name
\end{flushleft}
\end{minipage}
~
\begin{minipage}[t]{0.4\textwidth}
\begin{flushright} \large
\emph{Assessor:} \\
Dr. R.G.F. \textsc{Winkels}\\
\end{flushright}
\end{minipage}\\[2cm]

%----------------------------------------------------------------------------------------
%	LOGO SECTION
%----------------------------------------------------------------------------------------

%\framebox{\rule{0pt}{2.5cm}\rule{2.5cm}{0pt}}\\[0.5cm]
%\includegraphics[width=2.5cm]{figure}\\ % Include a department/university logo - this will require the graphicx package
%\textsc{\large \red{institute name}}\\[1.0cm] % 
 
%----------------------------------------------------------------------------------------

\vfill % Fill the rest of the page with whitespace

\end{titlepage}

\tableofcontents

\clearpage

\begin{abstract}
Storytelling is an important part of human culture, and automated storytelling has
been a popular topic in artificial intelligence research for a long time. However, the
focus of the field has been on generation, as opposed to rule extraction from
stories.
This thesis aims to develop a representation of stories to facilitate learning the structure of stories, in
such a way that these existing stories can be used to generate new stories in
the future. This thesis also provides a method for extracting rules from stories
represented according to this representation. This focus on rule extraction
rather than story generation is novel, since most other literature focusses on
story generation from a human-made and static rule set. When trained on two fairy tales
(``The Frog Prince'' and ``Caliph Stork'') the program was able to capture the rules of the stories and was
able to generalize over multiple occurences of the same event type in different stories (in this case, magical
transformation from animal to human and vice versa).

\end{abstract}

\input{sections/intro.tex}
\input{sections/related_work.tex}
\input{sections/story_structure.tex}
\input{sections/model.tex}
\input{sections/eval.tex}
\input{sections/discussion.tex}
\input{sections/future_work.tex}

\bibliographystyle{plain}
\bibliography{thesis.bib}
\clearpage

\begin{appendices}
\input{sections/appendices.tex}
\end{appendices}

\end{document}
